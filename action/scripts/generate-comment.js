#!/usr/bin/env node
/**
 * Generate a PR comment body for ARI scan results.
 *
 * Environment variables:
 *   ARI_PR_SCAN   — path to PR branch scan JSON
 *   ARI_BASE_SCAN — path to base branch scan JSON (optional, for delta)
 *   ARI_DELTA     — numeric delta value (optional)
 *   ARI_BASE_SCORE — base branch score (optional)
 *
...
if (outputFile) {
  const delimiter = `ARICOMMENT_${Date.now()}`;
