...
1. **Zero-friction adoption:** `npx ariscan .` must work on any repository without requiring the target repo's language toolchain, global installs, or configuration.
...
|---|---|---|
| **Runtime** | Node.js 22 + TypeScript 5.7 (strict) | Same as ripple-next. `npx ariscan .` = zero install. Widest reach via npm. Pin exact version in `.nvmrc` and `engines` field. |
...
**Implementation:** Taxonomy stored as structured JSON in `docs/error-taxonomy.json`, consumable via `npx ariscan taxonomy --json`. Each code includes: severity, pillar, description, remediation action, research citation.
...
npx ariscan . --format json

# GitHub Code Scanning integration
npx ariscan . --format sarif --output ariscan.sarif

# Self-check (modeled on ripple-next's pnpm doctor)
npx ariscan doctor --json

# Taxonomy lookup
npx ariscan taxonomy ARI-TST-001 --json

# Schema export for validation tooling
npx ariscan --json-schema
```
...
| `pnpm verify --json` | `npx ariscan verify --json` | Structured quality gate execution |
| `pnpm doctor --json` | `npx ariscan doctor --json` | Environment self-check |
...
|---|---|
| **Install** | `npx ariscan .` — no global install, no brew, no pip, no cargo. npm is the universal distribution channel. |
