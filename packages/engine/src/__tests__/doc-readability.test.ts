...
describe("docReadabilityAnalyzer (P5)", () => {
  ...
  describe("ARI-DOC-007: Security policy detection", () => {
    it("adds +5 for SECURITY.md", async () => {
      const ctx = createMockContext({
    ...
    it("emits ARI-DOC-007 when no security policy exists", async () => {
      const ctx = createMockContext({});
      const result = await docReadabilityAnalyzer.analyze(ctx);
      const finding = result.findings.find((f) => f.code === "ARI-DOC-007");
      expect(finding).toBeDefined();
      expect(finding?.severity).toBe("low");
      expect(finding?.remediation).toBeDefined();
    });
