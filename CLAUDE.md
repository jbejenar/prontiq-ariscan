...
- **Schema-first** — all types (PillarId, Finding, PillarResult) live in `@prontiq/ariscan-schema` and are Zod-validated. Engine and CLI depend on schema, never the reverse.
...
- **Running single test:** `pnpm --filter @prontiq/ariscan-engine test -- --run context-quality`
...
2. **Build before test** — the engine imports from `@prontiq/ariscan-schema` via its built output. Run `pnpm build` after schema changes before testing engine.
